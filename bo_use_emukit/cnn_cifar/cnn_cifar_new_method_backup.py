...
def evaluate(params):
    ...
    train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=batch_size, sampler=train_sampler, collate_fn=lambda x: tuple(x_.to(device) for x_ in default_collate(x)))
    validation_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=1024,
                                                    sampler=validation_sampler, collate_fn=lambda x: tuple(x_.to(device) for x_ in default_collate(x)))
    ...
    if training_time < 250:
        constraint_value = constraint_value + (training_time - 250)
        satisfied_constraints = satisfied_constraints + 1

    if model_size < 50:
        constraint_value = constraint_value + (model_size - 50)
        satisfied_constraints = satisfied_constraints + 1
    ...
    print('train_accuracy: %s validation_accracy: %s test_accuracy: %s constraint_value: %s training time: %s model size: %s' % (
        train_accuracy, validation_accuracy, test_accuracy, constraint_value, training_time, model_size))
    # validation_accuracy = 0.001
    return 1 - validation_accuracy, test_accuracy, constraint_value, time_stamp, training_time, model_size
...
def init():
    ...
    init_num_training_data = np.random.randint(low=5000, high=40000, size=(5, 1))
...
def create_config_space():
    ...
    num_training_data = ContinuousParameter('num_training_data', 5000, 40000)
...
if __name__ == '__main__':
    ...
    for _ in range(n_iterations + 1):
        ...
        _loss, test_accuracy, c_value, time_stamp, training_time, model_size = evaluate(new_config)
    ...
    np.save('./running_results_new_cnn6_cifar10_250_50_2.npy', record_results)
