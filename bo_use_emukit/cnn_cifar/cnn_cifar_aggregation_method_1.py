...
def evaluate(params):
    ...
    train_data, valid_data = train_test_split(train_dataset, train_size=config['num_training_data'], test_size=5000,
    ...
    if training_time < 125:
        constraint_value = constraint_value + (training_time - 125)
        satisfied_constraints = satisfied_constraints + 1

    if model_size < 20:
        constraint_value = constraint_value + (model_size - 20)
        satisfied_constraints = satisfied_constraints + 1
...
def create_config_space():
    ...
    max_num_training_data = np.log(40000)
    min_num_training_data = np.log(5000)
...
if __name__ == '__main__':
    ...
    for _ in range(n_iterations + 1):
        ...
        print("training instances:", new_config[10])
        ...
        if record_time > 15000:
            print("---------------   time out   ------------------")
            break

    np.save('./running_results_new_cnn6_cifar_125_20_3.npy', record_results)
