...
def evaluate(params):
    ...
    return 1 - balanced_acc_valid, balanced_acc_test, training_time, time_stamp, model_size
...
def init():
    ...
    for i in init_config:
        loss, balanced_accuracy, training_time, time_stamp, model_size = evaluate(i)
...
if __name__ == '__main__':
    ...
    init_c_training_time = init_c_training_time - 100
    init_c_model_size = init_c_model_size - 10
    ...
    for _ in range(n_iterations + 1):
        ...
        _loss, balanced_accuracy, training_time, time_stamp, model_size = evaluate(new_config)
        ...
        c_training_time = c_training_time - 100
        c_model_size = c_model_size - 10
        print(c_training_time, c_model_size)
        if (training_time < 100): #and (model_size < 10):
            constraint_results = np.vstack((constraint_results, np.array([_loss, balanced_accuracy, training_time, record_time, model_size, new_config], dtype=object)))
    ...
    np.save('./running_results_origin.npy', record_results)
