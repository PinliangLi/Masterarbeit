...
if __name__ == '__main__':
    ...
    init_c_model_size = init_c_model_size - 0.1
    ...
    bo = UnknownConstraintGPBayesianOptimization(variables_list=config_space, X=init_config, Y=init_loss,
                                                 Yc=init_c_model_size, batch_size=1)
    results = None
    best_result = np.inf
    ...
    for _ in range(n_iterations + 1):
        ...
        results = [UserFunctionResult(X_new[0], loss[0], Y_constraint=c_model_size[0])]

    np.save('./running_results_origin_cnn_MNIST_1_log_1.npy', record_results)
