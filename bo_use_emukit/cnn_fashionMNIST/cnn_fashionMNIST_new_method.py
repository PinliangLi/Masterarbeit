...
def evaluate(params):
    ...
    train_dataset = torchvision.datasets.FashionMNIST(root='../../data', train=True, transform=transforms.ToTensor(),
                                               download=True)
    ...
    if training_time < 1:
        constraint_value = constraint_value + (training_time - 1)
        satisfied_constraints = satisfied_constraints + 1

    if model_size < 0.5:
        constraint_value = constraint_value + (model_size - 0.5)
        satisfied_constraints = satisfied_constraints + 1
...
def create_config_space():
    ...
    num_training_data = ContinuousParameter('num_training_data', 1024, 8192)
...
if __name__ == '__main__':
    ...
    np.save('./running_results_new_cnn_FashionMNIST_1_0.5_1.npy', record_results)
