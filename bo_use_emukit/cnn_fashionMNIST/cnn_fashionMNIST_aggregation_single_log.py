...
def evaluate(params):
    ...
    if training_time < 0.2:
        constraint_value = constraint_value + (training_time - 0.2)
        satisfied_constraints = satisfied_constraints + 1

    if model_size < np.inf:
        #constraint_value = constraint_value + (model_size - 2.5)
        satisfied_constraints = satisfied_constraints + 1
...
if __name__ == '__main__':
    ...
    np.save('./running_results_new_cnn_FashionMNIST_10_2.5_1.npy', record_results)
