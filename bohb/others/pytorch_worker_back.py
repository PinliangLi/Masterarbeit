...
class PyTorchWorker(Worker):
    def __init__(self, N_train = 8192, N_valid = 1024, **kwargs):
            ...
            self.train_loader = torch.utils.data.DataLoader(dataset=self.train_dataset, batch_size=batch_size, sampler=train_sampler)
            self.validation_loader = torch.utils.data.DataLoader(dataset=self.train_dataset, batch_size=1024, sampler=validation_sampler)

            self.test_loader = torch.utils.data.DataLoader(dataset=self.test_dataset, batch_size=1024, shuffle=False)


    def compute(self, config, num_epoch, budget, working_directory, *args, **kwargs):
            """
            ...
            print(len(self.validation_loader.dataset))
...
if __name__ == "__main__":
    ...
    print(config)
    res = worker.compute(config=config, num_epoch=9, budget=2, working_directory='.')
    print(res)