...
class PyTorchWorker(Worker):
    ...
    def compute(self, config_id, config, budget, working_directory, *args, **kwargs):
        ...
        test_loader = torch.utils.data.DataLoader(dataset=self.test_dataset, batch_size=1024, shuffle=False, collate_fn=lambda x: tuple(
                x_.to(device) for x_ in default_collate(x)))
        ...
        if not model_parameters.is_file():
            ...
            load_time_1 = timeit.default_timer()
            print('loaded time : ', load_time_1 - load_time)
            print('----------------------------------------------')
        ...
        if megabytes_of_model > 500: #training_time > 25 or megabytes_of_model > 20:
            loss = np.inf
        else:
