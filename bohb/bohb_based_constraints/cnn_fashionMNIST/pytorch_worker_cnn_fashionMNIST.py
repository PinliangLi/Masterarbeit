...
class PyTorchWorker(Worker):
    ...
    def compute(self, config_id, config, budget, working_directory, *args, **kwargs):
        ...
        train_data, valid_data = train_test_split(self.train_dataset, train_size=num_train_data, test_size=self.N_valid, stratify=self.train_dataset.targets)
        ...
        if training_time > 0.2: #or megabytes_of_model > 0.5:
            loss = np.inf
        else:
            loss = 1 - validation_accuracy
            result = np.array([loss, config_id, test_accuracy, training_time, record_time, megabytes_of_model, config, len(train_loader.dataset)], dtype=object)
            # print(result)
