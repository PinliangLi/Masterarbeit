...
class PyTorchWorker(Worker):
    def __init__(self, N_train=8192, N_valid=1024, num_epoch=9, **kwargs):
        ...
        self.train_dataset = torchvision.datasets.FashionMNIST(root='../../../data', train=True, transform=transforms.ToTensor(),
                                                        download=True)
        self.test_dataset = torchvision.datasets.FashionMNIST(root='../../../data', train=False, transform=transforms.ToTensor())
    ...
    def compute(self, config_id, config, budget, working_directory, *args, **kwargs):
        ...
        if training_time > 10 or megabytes_of_model > 1:
            loss = np.inf
            _result = np.array(
                [1, test_accuracy, training_time, record_time, megabytes_of_model])
            _result = np.concatenate((_result, config_to_all_results))
            _result = np.concatenate((_result, np.array([len(train_loader.dataset), 10, 10, 1, 0])))
            ...
            result = np.array([loss, test_accuracy, training_time, record_time, megabytes_of_model, config,
                               len(train_loader.dataset)])
            # print(result)
            _result = np.array(
                [loss, test_accuracy, training_time, record_time, megabytes_of_model])
            _result = np.concatenate((_result, config_to_all_results))
            _result = np.concatenate((_result, np.array([len(train_loader.dataset), 10, 10, 1, 1])))
            all_results = np.load('./temp/all_results.npy', allow_pickle=True)
            all_results = np.vstack((all_results, _result))
            np.save('./temp/all_results', all_results)
            ...
            current_best_result[3] = record_time
